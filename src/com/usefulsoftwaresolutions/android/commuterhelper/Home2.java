...
import com.usefulsoftwaresolutions.android.commuterhelpertrial.util.IabHelper;
import com.usefulsoftwaresolutions.android.commuterhelpertrial.util.IabResult;
import com.usefulsoftwaresolutions.android.commuterhelpertrial.util.Inventory;
import com.usefulsoftwaresolutions.android.commuterhelpertrial.util.Purchase;


/*
 * To change from regular version to trial (and visa versa):
 * 		1. Right click on com.usefulsoftwaresolutions.android.commuterhelper, refactor, rename to com.usefulsoftwaresolutions.android.commuterhelper
        ...
        if (mHelper==null||!mHelper.handleActivityResult(requestCode, resultCode, data)) {		
			...
			case REQUEST_CODE_RECOVER_PLAY_SERVICES:
				if (resultCode == RESULT_CANCELED) {
					...
					builder.setTitle("Application Alarm")
							.setMessage(
			...
			if(mHomeManager.getSecurityManager().isTrialVersion()) {
				if (mHomeManager.getSecurityManager().hasExceededTrials()) {
					TrialVersionDialog tvdd=new TrialVersionDialog(
							...
							"Your trial period is over. In order to continue using Commuter Alarm you will have make one of the following purchases.",
							true,this
							);
...
//							R.style.AlertDialogCustomDark));
	...
	class ShowExpiredDialog extends AsyncTask<TrialVersionDialog,Void,TrialVersionDialog> {
		...
		protected void onPostExecute(TrialVersionDialog result) {
			result.show(getFragmentManager(), "Out of Trials");
		}
	}
    ...
    void verifyTurnOff() {
        ...
        bld.setMessage("Are you sure that you want to turn off the current alarm?");
